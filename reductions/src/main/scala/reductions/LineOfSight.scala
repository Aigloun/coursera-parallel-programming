...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    def lineOfSightHelper(i: Int, mx: Float): Unit = {
      if (i < input.length) {
        val maxAngle = max(input(i) / i, output(i-1))
        output(i) = maxAngle
        lineOfSightHelper(i + 1, maxAngle)
      }
    }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    def upsweepSequentialAcc(i: Int, mx: Float): Float = {
      if (i < until) {
        val maxAngle = max(input(i) / i, mx)
        upsweepSequentialAcc(i + 1, maxAngle)
      }
      else mx
    }

    upsweepSequentialAcc(from, 0)
  }
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    ...
    if (end - from <= threshold) Leaf(from, end, upsweepSequential(input, from, end))
      ...
      val (left, right) = parallel[Tree, Tree](upsweep(input, from, middle, threshold), upsweep(input, middle, end, threshold))
      Node(left, right)
    }
  }
